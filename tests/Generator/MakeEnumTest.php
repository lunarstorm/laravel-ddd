...
it('can generate enums', function ($domainPath, $domainRoot) {
    Config::set('ddd.domain_path', $domainPath);
    Config::set('ddd.domain_namespace', $domainRoot);

    $domain = new Domain('Invoicing');
    $name = 'InvoiceStatus';
    $relativePath = $domain->enum($name)->path;
    $expectedNamespace = $domain->namespace->enums;
    ...
    Artisan::call("make:enum {$domain->domain}:{$name}");
