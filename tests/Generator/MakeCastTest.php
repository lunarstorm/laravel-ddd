...
it('can generate casts', function ($domainPath, $domainRoot) {
    Config::set('ddd.domain_path', $domainPath);
    Config::set('ddd.domain_namespace', $domainRoot);

    $domain = new Domain('Invoicing');
    $name = 'SubtotalCast';
    $relativePath = $domain->cast($name)->path;
    $expectedNamespace = $domain->namespace->casts;
    ...
    Artisan::call("make:cast {$domain->domain}:{$name}");
