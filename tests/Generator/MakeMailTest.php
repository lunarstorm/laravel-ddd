...
it('can generate mailables', function ($domainPath, $domainRoot) {
    Config::set('ddd.domain_path', $domainPath);
    Config::set('ddd.domain_namespace', $domainRoot);

    $domain = new Domain('Invoicing');
    $name = 'InvoiceDueEmail';
    $relativePath = $domain->mail($name)->path;
    $expectedPath = base_path($relativePath);
    $expectedNamespace = $domain->namespace->mail;
    ...
    Artisan::call("make:mail {$domain->domain}:{$name}");
